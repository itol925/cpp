...
namespace demo::trans {
    ...
    public:
        ...
        Transaction(std::string  type, std::string  name, TransactionCallbackT cb_t) noexcept
            : type_(std::move(type)), name_(std::move(name)),  cb_t_(std::move(cb_t)) {
            data_ = T();
            is_done_ = false;
            is_owner_ = true;
        ...
        // 默认上报 raptor
        Transaction(std::string type, std::string name, TransactionCallback cb = cbDefault) noexcept
            : type_(std::move(type)), name_(std::move(name)), cb_(std::move(cb)) {
            is_done_ = false;
            is_owner_ = true;
            start_time_ = high_resolution_clock::now();
        }
